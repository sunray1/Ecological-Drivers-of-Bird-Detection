...
Load saved data instead of rerunning
```{r}
baseline_results_path <- file.path("data", "baseline_results.RData")
load(baseline_results_path)
```
...
baseline_results_path <- file.path("data", "baseline_results.RData")
save(baseline_results_df, file = baseline_results_path)
...
Load saved data instead of rerunning
```{r}
covariate_results_path <- file.path("data", "covariate_results.RData")
load(covariate_results_path)
```
...
covariate_results_path <- file.path("data", "covariate_results.RData")
save(covariate_results_df, file = covariate_results_path)
...
combined_results <- combined_results[
  !names(combined_results) %in% c("Chamaea fasciata",  #this has only one detection ever
                                  ...
                                  "Cyrtonyx montezumae", #this only has two detections
...
#sp_batch <- c("Passerculus sandwichensis") #for single species plot
...
p <- ggplot(pred_all, aes(x = .data[[covtoplot]], y = est)) +
  geom_line() +
...
ggsave(
  filename = paste("outputs/", covtoplot, "_pos.png", sep=""),
  plot = p,
  width = 10,
  height = 8,
  units = "in",
...
```{r}
View(combined_results$`Cyanocitta stelleri`)

View(covariate_results_df$humidityMax$estimates$`Quiscalus major`)
View(covariate_results_df$humidityMax$detection_histories$`Cyanocitta stelleri`)
...
# Save Outputs
```{r}
write.csv(cov_summary, "outputs/cov_summary.csv", row.names = FALSE)
```
