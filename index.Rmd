...
species_ci_excl0 <- names(combined_results)[
...
start <- 19
sp_batch <- species_ci_excl0[start:min(start + 8, length(species_ci_excl0))]
sp_batch <- c("Quiscalus major")
...
Figure out which species has the highest/lowest value of a covariate
...
# species with the max / min value
max_species <- names(which.max(vals))
min_species <- names(which.min(vals))
...
# optional: see the top 10 extremes
...
```{r}
View(combined_results$`Quiscalus major`)

View(covariate_results_df$humidityMax$estimates$`Quiscalus major`)
View(covariate_results_df$humidityMax$detection_histories$`Quiscalus major`)
```
